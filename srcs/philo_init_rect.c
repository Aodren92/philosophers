...
static void	philo_init_rect_philo_next(t_env *env)
{
...
static void	philo_init_rect_philo(t_env *env)
{
	...
	env->texture[4].pos = 4;
	philo_init_rect_philo_next(env);
}

static void	philo_init_rect_baguette_next(t_env *env)
{
...
void		philo_init_rect_baguette(t_env *env)
{
	...
	philo_init_rect_baguette_next(env);
}

t_err		philo_init_rect(t_env *env)
{
	philo_init_rect_philo(env);
	philo_init_rect_baguette(env);
	return (NONE);
}
